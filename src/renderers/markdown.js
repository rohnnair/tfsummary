...
function renderMarkdown(resources, options = {}) {
  ...
  if (totalMonthlyCost > 0) {
    lines.push(`**Estimated monthly cost: $${totalMonthlyCost.toFixed(2)}/mo**`);
    lines.push('');
  }

  // Resource table
  if (!options.summaryOnly) {
    const hasCosts = resources.some(r => r.monthlyCost !== null);
    ...
    for (const r of resources) {
      ...
      if (hasCosts) {
        ...
        lines.push(`| ${emoji} ${r.actionLabel} | \`${r.address}\` |`);
      }
    }
    lines.push('');
  ...
  lines.push('---');
  lines.push('*Generated by [tfsummary](https://github.com/YOUR_USERNAME/tfsummary)*');
  lines.push('');
